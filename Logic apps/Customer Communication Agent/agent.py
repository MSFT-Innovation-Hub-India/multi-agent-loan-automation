...
def interact_with_loan_application_agent():
    ...
    print("Available stages: Stage 1-6 (Application, Document Submission, Verification, Document Approval, Approval, Loan Letter)")
    ...
    1. Send Mail Tool (PRIMARY FUNCTION):
       ...
       - Stage mapping:
         ...
         * If customer mentions "stage 6" or "loan letter", use "letter"
      ...
      3. Conversation Flow:
       ...
       - Use the send_mail tool once you have all required information (use remembered customer ID)
       - Provide the standard response after successful email sending
       - For subsequent requests in the same conversation, only ask for the stage (and condition if Stage 4)
    
    Remember: Your primary goal is to collect customer ID ONCE per conversation, then for each request collect stage information and condition (only for Stage 4), then use the send_mail tool to notify them via email. DO NOT ask for customer ID repeatedly if already provided.
    """
