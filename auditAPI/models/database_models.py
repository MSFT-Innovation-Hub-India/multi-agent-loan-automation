...
class MasterCustomerData(Base):
    ...
    audit_records = relationship("AuditRecords", back_populates="customer")
...
class AuditRecords(Base):
    __tablename__ = "Audit_records"

    Audit_ID = Column(Integer, primary_key=True)
    Customer_ID = Column(String(20), ForeignKey('Master_Customer_Data.Customer_ID'))
    ...
    Remarks = Column(Text)
    Follow_Up_Required = Column(String(10))
    IsActive = Column(Integer, default=1)  # For BIT field

    # Relationship
    customer = relationship("MasterCustomerData", back_populates="audit_records")
