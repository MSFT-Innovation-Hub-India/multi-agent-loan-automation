...
def create_send_email_template_tool() -> Dict[str, Any]:
    """
    Create a tool definition for sending email templates.
    
    This tool should be used when sending templated emails to customers at different stages
    of the loan process. It uses predefined email templates with proper formatting.
    Works similar to send_mail but uses email templates.
    ...
    tool_definition = {
        ...
        "function": {
            ...
            "description": """Send a templated email for customer communication during loan process stages.
            This function works like send_mail but uses email templates to send professional formatted emails.
            It takes customer ID and stage (like '1', '2', 'application', etc.), maps to template, and sends via Logic App.
            Available stages: 1, 2, 3, 4, 5, 6 or application, document_submission, verification, document_approval, approval, loan_application_number.""",
            "parameters": {
                "type": "object",
                "properties": {
                    "customer_id": {
                        "type": "string",
                        "description": "The customer's unique ID number (required)"
                    },
                    "stage": {
                        "type": "string",
                        "description": "The loan process stage (required). Can be stage number like '1', '2', '3', '4', '5', '6' or stage name like 'application', 'document_submission', 'verification', 'document_approval', 'approval', 'loan_application_number'"
                    }
                },
                "required": ["customer_id", "stage"]
            }
...
def get_all_custom_agent_tools() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        create_send_email_template_tool()
    ]